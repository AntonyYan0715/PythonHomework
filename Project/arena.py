...
class Trader(MyThread):
    ...
    def place_limit_order(self, quantity=None, price=None, side=None):
        ...
        # if order.side==OrderSide.BUY:
        #     self.book_position+=order.quantity
        # else:
        #     self.book_position-=order.quantity
        return (1,self.id,order) ## check with TA
            
        #Limited order needs id,symbol,quantity,price,side,time
    ...
    def amend_quantity(self, quantity=None):
        ...
        self.book_position -= 100
        return(2,self.id,100)
    ...
    # You need to process each order according to the type (by enum) given by the 'response'    variable
        if response[0] == 1:
            ...
            if o.side == OrderSide.BUY:
                self.balance_track.append(self.balance_track[-1]-o.price*o.quantity)
                self.book_position += o.quantity
            else:
                self.balance_track.append(self.balance_track[-1] + o.price * o.quantity)
                self.book_position -= o.quantity
        #print(self.balance_track[-1],self.book_position)
    ...
    def random_action(self):
        ...
        quantity = random.randint(2,2)*100
        price = random.randint(2,2)*100
        if action == 1:
            if type == 1:
                if side == 1:
                    return self.place_limit_order(price,quantity,OrderSide.BUY)
                else:
                    return self.place_limit_order(price,quantity, OrderSide.SELL)
            elif type == 2:
                if side == 1:
                    return self.place_market_order(price, OrderSide.BUY)
                else:
                    return self.place_market_order(price, OrderSide.SELL)
            elif type == 3:
                if side == 1:
                    return self.place_ioc_order(price,quantity,OrderSide.BUY)
                else:
                    return self.place_ioc_order(price,quantity, OrderSide.SELL)
        # elif action == 2:
        #     return self.amend_quantity(quantity)
        # elif action == 3:
    ...
    def run_infinite_loop(self):
        ...
        # for i in range(0,10):
        #     self.random_action()
            # while len(exchange_to_trader[self.id]) != 0:
            #     self.process_response(exchange_to_trader[self.id].pop())
        while len(exchange_to_trader[self.id])!= 0:
            response = exchange_to_trader[self.id].pop()
            #print(len(exchange_to_trader[self.id]))
            self.process_response(response)
        if self.balance_track[-1] <= 0:
            self.is_started = False
        else:
            trader_to_exchange.appendleft(self.random_action())
        while len(exchange_to_trader[self.id])!= 0:
            response = exchange_to_trader[self.id].pop()
            #print(len(exchange_to_trader[self.id]))
            self.process_response(response)
...
class Exchange(MyThread):
    ...
    def place_new_order(self, order):
        ...
        for o in filled_orders:
            ...
            if o.side == OrderSide.BUY:
                self.balance[o.id] -= o.price*o.quantity
    ...
    def handle_request(self, request):
        ...
        if request[0] == 1:
            ...
            for r in result:
                exchange_to_trader[r[0]].appendleft(r[1])
        elif request[0] == 2:
            result = self.amend_quantity(request[1],request[2])
            exchange_to_trader[request[1]].appendleft(result)
        elif request[0] == 3:
    ...
    def run_infinite_loop(self):
        ...
        while len(trader_to_exchange) != 0:
            ...
            #print("Remaining task on exchange: "+str(len(trader_to_exchange)))
